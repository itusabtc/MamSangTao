...
import { getChatGPTUser, requireChatGPTUser, chatGPTSignOutPath } from '@/app/chatgpt-auth'
import { profileData, type Profile } from '@/lib/community-db'
import { ProfileCommunity } from '@/components/profile-community'
import { ProfileShowcase } from '@/components/profile-showcase'
import { AgentChallengePanel } from '@/components/agent-challenge-panel'
...
const localProfile: Profile = { user_id: 'local-preview', email: 'be@local.test', display_name: 'Nhà sáng tạo nhí', username: 'mam_local', avatar: '🌱', bio: 'Hồ sơ xem thử trên thiết bị này.', xp: 120, seeds: 300, streak: 1, joined_at: new Date().toISOString() }
const localData = { profile: localProfile, following: [] as Profile[], followers: 0, challenges: [] as Array<{id:number;course_slug:string;status:string;display_name:string;avatar:string}> }
...
export default async function ProfilePage() {
  ...
  const user = signedInUser || (isLocal ? null : await requireChatGPTUser('/ho-so'))
  const data = user ? await profileData(user) : localData
  ...
  return <><SiteHeader variant="page"/><main className="wrap py-8">
    ...
    <ProfileShowcase/>
    <div className="mt-7"><AgentChallengePanel/></div>
    <div className="mt-7 grid gap-7 lg:grid-cols-[1.1fr_.9fr]"><ProfileCommunity localPreview={!user}/><aside className="space-y-5">
      ...
      <section className="rounded-[28px] bg-blue p-5 text-white"><h2 className="flex items-center gap-2 text-2xl text-white"><Swords/>Lời thách đấu</h2><div className="mt-4 space-y-2">{data.challenges.map(item=><div key={item.id} className="rounded-2xl bg-white/15 p-3"><strong>{item.avatar} {item.display_name}</strong><p className="text-sm text-white/75">Mời bé thi đấu cờ vua · {item.status}</p></div>)}{!data.challenges.length?<p className="text-sm text-white/75">Chưa có lời thách đấu mới. Bé có thể đấu Agent máy ngay phía trên.</p>:null}</div></section>
    </aside></div>
  </main><SiteFooter/></>
}
