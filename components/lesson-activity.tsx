...
function ChessPractice({ lessonIndex }: { lessonIndex: number }) {
  ...
  return <div className="flex h-full min-h-0 flex-col"><div className="mx-auto grid aspect-square max-h-[min(58dvh,620px)] w-auto max-w-full flex-none grid-cols-8 overflow-hidden rounded-2xl border-4 border-[#8b5a2b] shadow-xl">{board.map((row,r)=>row.map((piece,c)=><button key={`${r}-${c}`} type="button" onClick={()=>click(r,c)} aria-label={`Ô ${String.fromCharCode(97+c)}${8-r}${piece?`, quân ${piece}`:''}`} className={`grid aspect-square place-items-center text-[clamp(1.25rem,4.2vmin,2.7rem)] ${(r+c)%2?'bg-[#7b9b62]':'bg-[#f2e6c9]'} ${selected?.row===r&&selected.col===c?'ring-4 ring-inset ring-yellow':''}`}>{piece}</button>))}</div><p className={`mx-auto mt-3 w-full max-w-xl rounded-2xl px-4 py-2.5 text-center text-sm font-extrabold ${message.startsWith('❌')?'bg-coral/10 text-coral':message.startsWith('✅')?'bg-green-soft text-green':'bg-blue/5 text-blue'}`} role="status">{message}</p><div className="mt-2 flex flex-wrap items-center justify-center gap-3"><button type="button" onClick={()=>{setBoard(INITIAL.map(row=>[...row]));setSelected(null);setMessage('Bàn cờ đã được xếp lại. Hãy chọn một quân để thử.')}} className="btn min-h-10 border border-hairline bg-white"><RotateCcw className="h-4 w-4"/>Xếp lại bàn cờ</button><span className="text-xs text-ink/55">Bài {lessonIndex+1} · Thử chọn quân rồi chọn ô đích</span></div></div>
}
