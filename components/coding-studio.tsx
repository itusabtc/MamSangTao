...
type GameType = 'collect' | 'race' | 'treasure' | 'maze'
...
function gameFromIdea(idea: string): GameType {
...
export function CodingStudio({ idea, onReset }: Props) {
  ...
  const [gameType, setGameType] = useState<GameType>(() => gameFromIdea(idea))
  const theme = useMemo(() => themeFromIdea(idea), [idea])
  const goalIcon = gameType === 'race' ? '🏁' : gameType === 'treasure' ? '🧰' : gameType === 'maze' ? '🗝️' : theme.item
  ...
  useEffect(() => {
    ...
    function keydown(event: KeyboardEvent) {
      ...
      setPosition(next)
      const reached = gameType === 'race' ? next.x >= 88 : Math.abs(next.x - star.x) <= 9 && Math.abs(next.y - star.y) <= 12
      if (reached) {
        setScore((value) => value + 1); playTone(880, 0.16)
        if (gameType === 'race') setPosition((value) => ({ ...value, x: 8 }))
  ...
  function resetStage() {
    runToken.current += 1; setRunning(false); setActiveId(null); setSpeech(''); setStageColor(theme.sky); setPosition({ x: 25, y: 62, rotation: 0 })
  }
  ...
  return (
    <section ref={sectionRef} className={fullscreen ? 'fixed inset-0 z-[100] overflow-y-auto bg-[#eef5ef] p-3 sm:p-6' : 'mt-7 scroll-mt-24'} aria-labelledby="coding-title" aria-modal={fullscreen || undefined} role={fullscreen ? 'dialog' : undefined}>
      <div className={fullscreen ? 'mx-auto max-w-7xl rounded-3xl border border-hairline bg-white p-4 shadow-2xl sm:p-6' : ''}>
        ...
        {gameMode ? <div className="mt-4 rounded-2xl bg-green-soft p-3"><div className="flex flex-col gap-2 sm:flex-row sm:items-center"><p className="text-sm font-extrabold">Chọn loại game:</p><div className="flex flex-wrap gap-2">{GAMES.map((game) => <button key={game.id} type="button" onClick={() => { setGameType(game.id); setScore(0); setStar({ x: 72, y: 55 }); resetStage() }} aria-pressed={gameType === game.id} className={`min-h-10 rounded-full px-3 text-sm font-bold ${gameType === game.id ? 'bg-blue text-white' : 'bg-white text-ink'}`}>{game.icon} {game.label}</button>)}</div></div><p className="mt-2 text-xs text-ink/60">Prompt được nhận diện theo chủ đề <strong>{theme.label}</strong>; bé vẫn có thể đổi loại game ở đây.</p></div> : null}
