...
export function CodingStudio({ idea, initialGame, onReset }: Props) {
  ...
  function resetStage() {
    runToken.current += 1; setRunning(false); setActiveId(null); setSpeech(''); setStageColor(theme.sky); setFacing('right'); setPosition({ x: 25, y: gameType === 'flappy' ? 48 : 62, rotation: 0 }); setPipeX(88)
  }

  async function run() {
    const token = ++runToken.current
    setRunning(true); setSpeech(''); setFacing('right'); setPosition({ x: 25, y: 62, rotation: 0 })
    for (const command of commands) {
      if (token !== runToken.current) return
      setActiveId(command.id)
      if (command.type === 'move') { setFacing('right'); setPosition((p) => ({ ...p, x: Math.min(88, p.x + 16) })) }
      if (command.type === 'back') { setFacing('left'); setPosition((p) => ({ ...p, x: Math.max(8, p.x - 16) })) }
