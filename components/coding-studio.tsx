...
export function CodingStudio({ idea, onReset }: Props) {
  ...
  useEffect(() => {
    ...
    function keydown(event: KeyboardEvent) {
      ...
      setPosition(next)
      if (Math.abs(next.x - star.x) <= 9 && Math.abs(next.y - star.y) <= 12) {
        setScore((value) => value + 1); playTone(880, 0.16)
        setStar((value) => ({ x: value.x > 50 ? 20 : 78, y: value.y > 45 ? 28 : 62 }))
      }
    }
    window.addEventListener('keydown', keydown)
    return () => window.removeEventListener('keydown', keydown)
  }, [gameMode, playTone, position, star])
