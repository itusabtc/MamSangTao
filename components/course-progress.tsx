  ...
  return <section className="rounded-3xl border border-hairline bg-white p-5 shadow-sm sm:p-7" aria-labelledby="lesson-title">
    ...
    {active && selected !== null ? <div className="fixed inset-0 z-[110] bg-[#eef5ef]" role="dialog" aria-modal="true" aria-labelledby="lesson-dialog-title">
      <div className="grid h-dvh w-full grid-rows-[auto_minmax(0,1fr)_auto] bg-white">
        ...
        <footer className="z-20 flex items-center justify-between gap-3 border-t border-hairline bg-white px-3 py-2.5 sm:px-6"><button type="button" disabled={selected===0} onClick={()=>setSelected(value=>value===null?0:Math.max(0,value-1))} className="btn min-h-10 border border-hairline bg-white disabled:opacity-35"><ArrowLeft className="h-4 w-4"/><span className="hidden sm:inline">Bài trước</span></button><div className="flex items-center gap-2"><span className="text-sm font-extrabold text-ink/55">{selected+1} / {lessons.length}</span><div className="hidden h-2 w-28 overflow-hidden rounded-full bg-cream sm:block"><div className="h-full bg-blue" style={{width:`${((selected+1)/lessons.length)*100}%`}}/></div></div><button type="button" disabled={selected===lessons.length-1} onClick={()=>setSelected(value=>value===null?0:Math.min(lessons.length-1,value+1))} className="btn btn-primary min-h-10"><span className="hidden sm:inline">Bài tiếp</span><ArrowRight className="h-4 w-4"/></button></footer>
      </div>
    </div> : null}
  </section>
}
