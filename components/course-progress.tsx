...
export function CourseProgress({ lessons, courseSlug, courseIcon }: { lessons: readonly Lesson[]; courseSlug: string; courseIcon: string }) {
  ...
  function speakLesson(turnOn=true){if(!active||typeof window==='undefined'||!('speechSynthesis' in window))return;window.speechSynthesis.cancel();if(!turnOn)return;const speech=new SpeechSynthesisUtterance(`${active.title}. ${active.goal}. ${active.steps.join('. ')}`);speech.lang='vi-VN';speech.rate=.88;speech.pitch=1.08;window.speechSynthesis.speak(speech)}
    ...
    {active && selected !== null ? <div className="fixed inset-0 z-[110] bg-[#eef5ef]" role="dialog" aria-modal="true" aria-labelledby="lesson-dialog-title">
      <div className="grid h-dvh w-full grid-rows-[auto_minmax(0,1fr)] bg-white">
        ...
        <div className="min-h-0 overflow-y-auto lg:grid lg:grid-cols-[minmax(0,1.12fr)_minmax(380px,.88fr)] lg:overflow-hidden">
          <div className="relative min-h-0 bg-cream/60 p-1 sm:p-2 lg:overflow-hidden"><div className="h-full min-h-[520px]"><LessonActivity courseSlug={courseSlug} lessonIndex={selected}/></div><div className="absolute left-3 top-3 z-10 flex max-w-[min(390px,70%)] items-center gap-2 rounded-2xl border border-violet/20 bg-white/95 p-2 pr-3 shadow-lg backdrop-blur"><span className="grid h-11 w-11 shrink-0 place-items-center rounded-xl bg-violet/10 text-2xl">🦉</span><p className="line-clamp-2 flex-1 text-xs font-bold text-ink/75 sm:text-sm">{active.steps[0]}</p><button type="button" onClick={toggleVoice} aria-pressed={voiceOn} aria-label={voiceOn?'Tắt hướng dẫn tiếng Việt':'Nghe hướng dẫn tiếng Việt'} className={`grid h-10 w-10 shrink-0 place-items-center rounded-full ${voiceOn?'bg-violet text-white':'bg-violet/10 text-violet'}`}>{voiceOn?<Volume2 className="h-5 w-5"/>:<VolumeX className="h-5 w-5"/>}</button></div></div>
