...
export type Profile={user_id:string;email:string;display_name:string;username:string;avatar:string;bio:string;xp:number;seeds:number;streak:number;joined_at:string}
const statements=[
...
export async function leaderboardProfiles(limit=20){await ready();return (await env.DB.prepare(`SELECT user_id,display_name,username,avatar,bio,xp,seeds,streak,joined_at,email FROM profiles ORDER BY xp DESC,streak DESC,display_name ASC LIMIT ?`).bind(limit).all()).results as unknown as Profile[]}
